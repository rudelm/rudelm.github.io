...
## Setup
The GIST recommended to just download the acme.sh script and to use it in standalone mode, which I wanted to avoid by using the Porkbun DNS API. So we'll combine my [previous experience]({{< ref "/posts/2023/10/12/configure-lets-encrypt-acme-with-ionos-api-in-openwrt" >}}) with acme and the IONOS DNS API.
